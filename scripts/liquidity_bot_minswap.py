...
class liquidityBotConfig(BaseClientModel):
    script_file_name: str = Field(default="liquidity_bot_minswap.py")
    ...
    trading_pair: str = Field(
        "ADA-MIN",
        client_data=ClientFieldData(
            prompt_on_new=True, prompt=lambda mi: "Trading pair (e.g., ADA-MIN)"
        )
    )
    ...
    token1: str = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Token to Add or remove from liquidity pool (e.g., MIN)" if mi.side.upper() == "ADD" else None
        ),
    )
    ...
    amount0: int = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Amount to Add or remove from liquidity pool (e.g., 500)" if mi.side.upper() == "ADD" else None
        ),
    )
...
class liquidityBotMinswap(ScriptStrategyBase):
    on_going_task = False
    ...
    async def async_task(self):
        ...
        try:
            ...
            price_ADA_MIN = Decimal(price_data["prices"][1])  # Assuming ADA/MIN price

            amount1 = (Decimal(amount0) * price_ADA_MIN).quantize(Decimal("1."), rounding=ROUND_DOWN)
            if lp_type == LPType.ADD:
            ...
            await self.update_balance(chain, network, address, token0, token1)

            # Fetch current price of the liquidity pool
            self.logger().info(f"Fetching price from liquidity pool {self.config.trading_pair}...")
            price_MIN_ADA = Decimal(price_data["prices"][0])

            self.logger().info(f"Price MIN/ADA: {price_MIN_ADA}, ADA/MIN: {price_ADA_MIN}")
