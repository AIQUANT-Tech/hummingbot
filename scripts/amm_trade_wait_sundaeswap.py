...
class AmmTradeConfig(BaseClientModel):
    script_file_name: str = Field(default="amm_trade_wait_sundaeswap.py")
    ...
    price_range: Decimal = Field(
        Decimal("0.000095"),
        client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Price of SBERRY-ADA in which you want to buy or sell.")
    )


class AmmTradeWaitSundaeswap(ScriptStrategyBase):
    on_going_task = False
