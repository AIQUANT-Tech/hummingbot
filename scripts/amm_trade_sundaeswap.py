...
class AmmTradeConfig(BaseClientModel):
    script_file_name: str = Field(default="amm_trade_sundaeswap.py")
...
class AmmTradeSundaeswap(ScriptStrategyBase):
            ...
            while True:
                try:
                    ...
                    if status == "confirmed":
                        ...
                        if valid_contract:
                            self.logger().info("Transaction is valid and successfully processed.")
                        else:
                            self.logger().warning("Transaction was processed but is invalid.")
