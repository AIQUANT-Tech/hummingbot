...
class AmmPriceConfig(BaseClientModel):
    script_file_name: str = Field(default="amm_price_sundaeswap.py")
    ...
    trading_pair: str = Field(
        "SBERRY-ADA",
        client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Trading pair (e.g., SBERRY-ADA)")
    )
    side: str = Field(
        "BUY",
...
class AmmPriceSundaeswap(ScriptStrategyBase):
    on_going_task = False
    ...
    async def async_task(self):
        ...
        try:
            ...
            expectedAmount = data.get("expectedAmount", "N/A")
            self.logger().info(f"Price: {price}, expectedAmount: ₳{expectedAmount}, rawAmount:{rawAmount} lovelace, amount:{amount}")
